...
- **카드**: `aspect-ratio: 1/1`, `border-radius: 14px`, `padding: 16px`, 부드러운 그림자 `0 2px 8px rgba(40,40,80,0.04)`. 제목 13px / weight 600. 본문 12px, opacity 0.72, 5줄 말줄임 처리
...
- 카드 본문은 `-webkit-line-clamp: 5`와 `text-overflow: ellipsis` 사용. 제목은 줄바꿈하지 않음 (한 줄)
- 카드 미리보기는 마크다운 소스를 plain text로 표시 — 형식 문자도 그대로. 마크다운은 모달의 보기 모드에서만 렌더링됨. 작은 카드의 시각적 노이즈를 줄이기 위함
